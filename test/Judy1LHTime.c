...
// Whole array!! -- MUST use -T0 option!!!!!!!!!!
    ...
    if (J1Flag)
    {
        ...
        int ret;
        TstKey = 0;
        ret = Judy1First(J1, &TstKey, PJE0);
        for (Hits1 = 0; ret; Hits1++)
            ret = Judy1Next(J1, &TstKey, PJE0);
        if (Hits1 != Elements)
            printf("\nHits1 = %ld, Elements = %ld, Diff = %ld\n", Hits1, Elements, Hits1 - Elements);
    }
    ...
    if (JLFlag)
    {
        ...
        PPvoid_t PPValue;
        TstKey = 0;
        PPValue = JudyLFirst(JL, &TstKey, PJE0);
        for (HitsL = 0; PPValue; HitsL++)
            PPValue = JudyLNext(JL, &TstKey, PJE0);
        if (HitsL != Elements)
            printf("\nHitsL = %ld, Elements = %ld, Diff = %ld\n", HitsL, Elements, HitsL - Elements);
    }
#endif  // DEBUG_COUNT_TIME
...
// TEST_COUNT_USING_JUDY_NEXT is incompatible with hFlag.
            ...
            while (elm < Elements)
            {
                ...
                if (Tit)
                {
                    ...
                    {
                        printf("\n");
                        printf("Count1 = %" PRIuPTR", TstKey = 0x%lx\n",
                               Count1, TstKey);
                        printf("Elements %zd\n", Elements);
                        FAILURE("J1C at", elm);
                    }
    ...
    if (JLFlag)
    {
        for (DminTime = 1e40, icnt = ICNT, lp = 0; lp < Loops; lp++)
        {
            ...
            if (VFlag && PValue && (*PValue != TstKey))
            {
                ...
                FAILURE("JudyLFirst ret bad *PValue at", elm);
            }
            elm = 0;
#else // TEST_COUNT_USING_JUDY_NEXT
            Word_t TstKey2;
            ...
            while (elm < Elements)
            {
                ...
                if (Tit)
                {
                    ...
                    if (CountL != (elm + 1))
                    {
                        printf("\nCountL = %" PRIuPTR", elm +1 = %" PRIuPTR"\n", CountL, elm + 1);
                        printf("TstKey =  0x%016lx\n", TstKey);
                        printf("Elements %zd, Delta = %ld IMMED\n", Elements, (long)(CountL - (elm + 1)));
                        FAILURE("JLC at", elm);
                    }
#else // TEST_COUNT_USING_JUDY_NEXT
                    if (TstKey > TstKey2) {
